...
#'
#' @param plot_design A \code{data.table} or \code{data.frame}. It most contain
#' three columns labeled: 'plot', 'row', and 'column'.
#'
#' @return A \code{data.table} without at the edge line of the plots.
#'
#' @export
remove_edge <- function(plot_design) {

  # Makes a copy
  frame <- as.data.table(plot_design)
  ...
  # Batch application
  edge_frame <- lapply(X = 1:length(plot_id),
                       FUN = edge,
                       plot_id = plot_id,
                       frame = frame)
